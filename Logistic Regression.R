...
xtabs(~ SalePrice + MSZoning , data=Train)
xtabs(~ SalePrice + Street  , data=Train)
xtabs(~ SalePrice + Neighborhood , data=Train)
xtabs(~ SalePrice + BldgType , data=Train)
xtabs(~ SalePrice + SaleType  , data=Train)


## Now we are ready for some logistic regression. First we'll create a very
## simple model that uses BldgTypwe to predict Price
xtabs(~ SalePrice + BldgType  , data=Train)
logistic_simple <- glm(SalePrice ~ BldgType, data=Train, family="binomial")
...
predicted.data <- data.frame(probability.of.SalePrice=logistic_simple$fitted.values,BldgType=Train$BldgType)
predicted.data

ggplot(data=predicted.data, aes(x= Train$BldgType, y=probability.of.SalePrice)) +
  stat_function(fun = function(x) 1/(1 + exp(-x)), geom = "line") +
  xlab("BldgType") +
  ylab("Predicted Sales")


xtabs(~ probability.of.SalePrice + BldgType, data=predicted.data)



logistic <- glm(SalePrice ~ BldgType + BldgType + MSZoning +Street + Neighborhood + SaleType, data=Train, family="binomial")
summary(logistic)
...
predicted.data <- data.frame(probability.of.SalePrice=logistic$fitted.values,SalePrice=Train$SalePrice)
predicted.data <- predicted.data[order(predicted.data$probability.of.SalePrice, decreasing=FALSE),]
